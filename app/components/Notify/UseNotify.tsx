...
const notifyReducer = (state: NotifyState, action: NotifyAction): NotifyState => {
    switch (action.type) {
        case 'OPEN_MODAL':
            return {
                isOpen: true,
...
const useNotify = (): {
    ...
    const NotificationComponent = (): JSX.Element => {
        return (
            <>
                {state.isOpen && <Notify inVariant={state.variant} inMessage={state.message} inClose={closeNotification} />}
            </>
        )
    };
